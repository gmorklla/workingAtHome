...
export class TreeComponent implements OnInit {
  ...
  addWindow(node) {
    ...
    this.service.addWindow(node, this.designId, winName).subscribe(val => {
  ...
  deleteWindow(node) {
    ...
    of(alerta)
      .pipe(
        ...
        tap(_ => {
          if (
            node.id === Number(this.route.snapshot.paramMap.get('windowId'))
          ) {
            this.navigateTo({
              id: this.treeData.id,
              designId: this.treeData.designId
            });
          }
        }),
